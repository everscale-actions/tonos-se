...
exports.getConfig = () => {
    return { 
        ...
        // "paramsStart": ["-c", path.join(appPath, "conf" , "nginx.conf"), "-g", `"pid ${pidFilePath};working_directory ${workingDir};error_log ${path.join(workingDir,"error.log")};"`],
        // "paramsStop": ["-c", path.join(appPath, "conf" , "nginx.conf"), "-s", "stop" ],
        "paramsStart": `-c ${path.join(appPath, "conf" , "nginx.conf")} -g "pid ${pidFilePath};working_directory ${workingDir};error_log ${path.join(workingDir,"error.log")};"`,
        "paramsStop": `-c ${path.join(appPath, "conf" , "nginx.conf")} -s stop -g "pid ${pidFilePath};"`,
