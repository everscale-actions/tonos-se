...
async function runService(binPath, binParams, cwd, workDir){
	// console.log(binParams);
	// if(!fs.existsSync(workDir)) { console.log(`Creating working directory: ${workDir}`); fs.mkdirSync(workDir); }
    // const pr = childProcess.spawn( binPath, binParams, { cwd: workDir });
    //pr.unref();
    //pr.unref();
	const command = `${binPath} ${binParams}`;
	// console.log(command)

	const process =  childProcess.execSync(command, {cwd: workDir});

	// process.on('error', err => {
	// 	console.error(`Failed to spawn NGINX process:\n${err}: ${err}`);
	//   });
	//console.log(process)
}
