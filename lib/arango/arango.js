...
async function start() {
  if (!fs.existsSync(moduleConfig.pidFilePath)) {
    ...
    fs.readdirSync(moduleConfig.migrationsPath).forEach((file) => {
        moduleConfig.paramsStartClient.push("--javascript.execute", path.join(moduleConfig.migrationsPath, file)); // add migration param 
        serverManagement.runApp(
            moduleConfig.binPathClient,
            moduleConfig.paramsStartClient,
            moduleConfig.workingDir,
            );
        //execSync(`${moduleConfig.binPathClient} -c none --server.authentication false --javascript.startup-directory ${path.join(moduleConfig.appPath, 'usr', 'share', 'arangodb3', 'js')} --server.endpoint=${process.env.ARANGO_ENDPOINT} --javascript.execute "${path.join(moduleConfig.migrationsPath, file)}"`);
    });
  } else {
